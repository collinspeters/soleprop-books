...
<div class="row mb-3">
    <div class="col-4 text-center">
        ...
        <h6 class="mb-0">{{ $summaryData['total_income'] ?? '0.00' }}</h6>
    </div>
    <div class="col-4 text-center">
        ...
        <h6 class="mb-0">{{ $summaryData['total_expense'] ?? '0.00' }}</h6>
    </div>
    <div class="col-4 text-center">
        <div class="{{ isset($summaryData['net_profit']) && str_contains($summaryData['net_profit'], '-') ? 'text-warning' : 'text-info' }}">
            <i class="fa {{ isset($summaryData['net_profit']) && str_contains($summaryData['net_profit'], '-') ? 'fa-minus' : 'fa-plus' }}"></i>
            <strong>{{ trans('Profit') }}</strong>
        </div>
        <h6 class="mb-0">{{ $summaryData['net_profit'] ?? '0.00' }}</h6>
    </div>
</div>

@if(isset($aiExplanation))
<!-- AI Explanation -->
...
<div class="border-top pt-3 mt-3">
    <div class="row">
        <div class="col-6">
            <a href="{{ route('reports.monthly-summary.show', [$year ?? date('Y'), $month ?? date('n')]) }}" class="btn btn-sm btn-primary btn-block">
